...
@pysnooper.snoop(output=nowTime+".analysisSanger.log",watch=("resultList","upStart","upEnd"))
    ...
    for f in p.iterdir():
        if f.is_dir():
            ...
            for eachFile in xlsxFiles:
                ...
                data['localPos'] = data['峰图位置']
                sampleInfos = data[["sampleId","chrom","pos","geneType","ab1Name","gene","fangx","localPos"]]
                for index, row in sampleInfos.iterrows():
                    sampleId = row['sampleId']
                    localPos = row['localPos']
                    chromId = row['chrom']
                    posId = row['pos']
                    ...
                    logging.info("'ab1':'{}'--'localPos':{}--'pngName':'{}'.".format(sampleAb1,localPos,pngName))
                    #运行程序
                    logging.info("开始Sanger截图...")
                    os.system("/usr/bin/Rscript /share_data/clin_result/clin_epilepsy_result/Sanger/PyWebIO_Sanger/sangerLocalPosPlot_linux_v2_pywebio.R %s %d %s " % (sampleAb1,localPos,pngName))
                    ...
                    if sampleId not in sampleDict:
                        ...
                        for i in localSampleList:
                            os.system("cp %s %s" % (i, sampleIdPath))
                        os.system("chmod 777 %s" % sampleIdPath)
                        os.system("chmod -R 777  %s/*" % sampleIdPath)
...
def parserStr(content):
    adjunct = re.findall(r'<li><a href=\"(.*)\">2022', content)
    if adjunct:
        for each in adjunct:
            #替换里面的字符串
